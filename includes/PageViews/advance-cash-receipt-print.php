...
if ((string) ($row['receipt_status'] ?? 'none') !== 'issued') {
    header('Location: ' . app_path('pages/advance-cash-view.php') . '?id=' . $id . '&error=receipt_not_issued');
    exit;
}
...
<div class="no-print">
    ...
    <a href="<?= htmlspecialchars(app_path('pages/advance-cash-view.php') . '?id=' . $id, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-light btn-sm ms-2">กลับหน้ารายละเอียด</a>
</div>

...
<div class="container mt-4 mb-5">
    ...
    <?php if (!empty($_GET['error'])): ?>
        <div class="alert alert-danger alert-dismissible fade show" role="alert">
            ...
            } elseif ($err === 'invalid_pr_status') {
                echo 'ส่งแจ้งเตือนได้เฉพาะใบ PR ที่สถานะ PENDING เท่านั้น';
            } else {
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="purchase-request-create.php" class="btn btn-orange rounded-pill px-4 shadow-sm">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group shadow-sm rounded">
                                    <a href="purchase-request-view.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-white text-primary border" title="ดูรายละเอียด">
                                        <i class="bi bi-eye-fill"></i>
                                    </a>

                                    <?php if($row['status'] == 'pending'): ?>
                                        <?php $lineSentOnce = trim((string) ($row['line_sent_at'] ?? '')) !== ''; ?>
                                        <a href="<?= htmlspecialchars(app_path('actions/action-handler.php')) ?>?action=send_pr_request&id=<?= (int) $row['id'] ?><?= htmlspecialchars($csrfQ, ENT_QUOTES, 'UTF-8') ?>"
                                           class="btn btn-sm btn-white text-warning border"
                                           title="<?= $lineSentOnce ? 'ส่งแจ้งเตือนซ้ำผ่าน LINE' : 'ส่งแจ้งเตือนผ่าน LINE เพื่อขออนุมัติ' ?>"
                                           onclick="return confirm('<?= $lineSentOnce ? 'ยืนยันการส่งแจ้งเตือนซ้ำไป LINE ?' : 'ยืนยันการส่งแจ้งเตือนไป LINE เพื่อขออนุมัติ ?' ?>')">
                                            <i class="bi bi-bell-fill"></i>
                                            <span class="d-none d-lg-inline ms-1"><?= $lineSentOnce ? 'ส่งซ้ำ' : 'แจ้งเตือน' ?></span>
                                        </a>
                                        <?php if ($canApprovePr): ?>

...
<div class="container mt-4 mb-5">
    <?php if ($errorCode === 'invalid_items'): ?>
        <div class="alert alert-warning alert-dismissible fade show" role="alert">
            กรุณาเพิ่มรายการสินค้าอย่างน้อย 1 รายการ และกรอกจำนวน/ราคาให้ถูกต้อง
    ...
    <form action="<?= htmlspecialchars(app_path('actions/action-handler.php')) ?>?action=create_po_direct" method="POST">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="fw-bold mb-0 page-title"><i class="bi bi-file-earmark-plus-fill text-primary me-2"></i> สร้างใบสั่งซื้อ (PO)</h3>
            <div>
                <a href="<?= htmlspecialchars(app_path('pages/purchase-order-list.php')) ?>" class="btn btn-light rounded-pill px-4 me-2">ยกเลิก</a>
                <button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm fw-bold">บันทึกใบ PO</button>
            </div>
        </div>
        ...
        <div class="row g-3 mb-4 card-soft p-3 mx-0 bg-white">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control bg-light fw-bold text-primary" value="<?= htmlspecialchars($po_number, ENT_QUOTES, 'UTF-8') ?>" readonly>
            </div>
            <div class="col-md-4">
                <label class="form-label fw-bold">วันที่ออกใบสั่งซื้อ *</label>
                <input type="date" class="form-control" name="issue_date" lang="en-GB" value="<?= htmlspecialchars(date('Y-m-d'), ENT_QUOTES, 'UTF-8') ?>" required>
            </div>
            <div class="col-md-4">
                <label class="form-label fw-bold">ผู้ขาย (Supplier)</label>
                <input type="text" id="supplier_search" class="form-control" list="supplier_list" placeholder="พิมพ์ชื่อผู้ขายเพื่อค้นหา (ถ้ามี)">
        ...
        <div class="card card-soft p-4 mb-4">
            ...
            <div class="row g-3">
                <div class="col-md-12">
                    ...
                    <input type="text" name="quotation_number" class="form-control" maxlength="120" placeholder="เช่น QT-2026-015 (ถ้ามี)">
                </div>
                <div class="col-md-12">
                    <label class="form-label fw-bold">หมายเหตุ (สำหรับแสดงในใบ PO)</label>
                    <textarea name="quotation_note" class="form-control" rows="2" maxlength="500" placeholder="เช่น เงื่อนไขจัดส่ง/เงื่อนไขพิเศษ"></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="card card-soft p-4 mb-4">
            ...
            <div class="row mt-3 g-3">
                <div class="col-md-6">
                    ...
                    <div class="mb-3 d-none" id="vat_mode_wrap">
                        ...
                        <select class="form-select form-select-sm" name="vat_mode" id="vat_mode" onchange="calculateTotal()">
                            ...
                            <option value="inclusive">VAT รวมในราคา (ถอด VAT ออก)</option>
                        </select>
                    </div>
...
function calculateTotal() {
    ...
    if (vatHelpText) {
        if (!vatOn) {
            ...
            vatHelpText.textContent = 'ยอดรายการถือว่ารวม VAT แล้ว ระบบถอด VAT 7% ออก (ตัวอย่าง 15,504 -> 14,489.72)';
        } else {

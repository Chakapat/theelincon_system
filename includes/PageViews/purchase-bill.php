...
$handler = app_path('actions/purchase-bill-handler.php');
...
foreach (Db::tableRows('purchase_bills') as $bill) {
    ...
    if ($billDate < $ymStart || $billDate > $ymEnd) {
        continue;
    }
    ...
    $sumTotal += (float) ($bill['total_amount'] ?? 0);
}
...
foreach ($rows as $row) {
    ...
    $siteExpenseSummary[$label]['total_amount'] += (float) ($row['total_amount'] ?? 0);
}
...
foreach (Db::tableRows('purchase_bills') as $billRow) {
    ...
    if (!str_starts_with($billNumber, $billPrefix)) {
        continue;
    }
...
$editLines = [];
if ($editId > 0) {
    $candidate = Db::row('purchase_bills', (string) $editId);
...
$formAction = $isEditing ? 'update' : 'save';
$currentBillNo = $isEditing ? (string) ($editBill['bill_number'] ?? $nextBillNo) : $nextBillNo;
$currentBillDate = $isEditing
    ? date('d/m/Y', strtotime((string) ($editBill['bill_date'] ?? date('Y-m-d'))))
    : date('d/m/Y');
$currentVendorName = $isEditing ? (string) ($editBill['vendor_name'] ?? '') : '';
$currentSiteId = $isEditing ? (int) ($editBill['site_id'] ?? 0) : 0;
$currentNote = $isEditing ? (string) ($editBill['note'] ?? '') : '';
...
<div class="container pb-5">
    <div class="card po-card mb-4">
        <div class="card-body p-0">
            ...
            <div class="p-4">
                <form method="post" action="<?= htmlspecialchars($handler, ENT_QUOTES, 'UTF-8') ?>?action=<?= htmlspecialchars($formAction, ENT_QUOTES, 'UTF-8') ?>" id="billForm" class="row g-3">
                    <?php csrf_field(); ?>
                    <input type="hidden" name="month" value="<?= htmlspecialchars($month, ENT_QUOTES, 'UTF-8') ?>">
                    <?php if ($isEditing): ?>
                        <input type="hidden" name="id" value="<?= (int) ($editBill['id'] ?? 0) ?>">
                    <?php endif; ?>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="bill_date" id="bill_date" class="form-control rounded-3" value="<?= htmlspecialchars($currentBillDate, ENT_QUOTES, 'UTF-8') ?>" placeholder="dd/mm/yyyy" pattern="^([0-2][0-9]|3[0-1])/(0[1-9]|1[0-2])/[0-9]{4}$" required>
                    </div>
                    <div class="col-md-5">
                        <label class="form-label small fw-bold">ผู้ขาย</label>
                        <input type="text" name="vendor_name" class="form-control rounded-3" list="vendor_list" maxlength="255" value="<?= htmlspecialchars($currentVendorName, ENT_QUOTES, 'UTF-8') ?>" required>
                                ...
                                <tbody id="lineBody">
                                    <?php if ($isEditing && count($editLines) > 0): ?>
                                        <?php foreach ($editLines as $idx => $line): ?>
                                            <tr class="line-row">
                                                ...
                                                <td><input type="text" class="form-control form-control-sm line-name" name="item_name[]" maxlength="500" value="<?= htmlspecialchars((string) ($line['item_name'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" required></td>
                                                <td><input type="number" class="form-control form-control-sm line-qty" name="item_qty[]" step="0.001" min="0" value="<?= htmlspecialchars((string) ($line['quantity'] ?? 0), ENT_QUOTES, 'UTF-8') ?>" required></td>
                                                <td><input type="text" class="form-control form-control-sm line-unit" name="item_unit[]" maxlength="40" value="<?= htmlspecialchars((string) ($line['unit'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"></td>
                                                <td><input type="number" class="form-control form-control-sm line-price" name="item_price[]" step="0.001" min="0" value="<?= htmlspecialchars((string) ($line['unit_price'] ?? 0), ENT_QUOTES, 'UTF-8') ?>" required></td>
                                                <td><input type="text" class="form-control form-control-sm line-discount" name="item_discount[]" maxlength="20" value="<?= htmlspecialchars((string) ($line['discount_input'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"></td>
                                    ...
                                    <?php else: ?>
                                        <tr class="line-row">
                                            ...
                                            <td><input type="text" class="form-control form-control-sm line-name" name="item_name[]" maxlength="500" required></td>
                                            <td><input type="number" class="form-control form-control-sm line-qty" name="item_qty[]" step="0.001" min="0" value="1" required></td>
                                            <td><input type="text" class="form-control form-control-sm line-unit" name="item_unit[]" maxlength="40"></td>
                                            <td><input type="number" class="form-control form-control-sm line-price" name="item_price[]" step="0.001" min="0" required></td>
                                            <td><input type="text" class="form-control form-control-sm line-discount" name="item_discount[]" maxlength="20"></td>
                                            <td><input type="text" class="form-control form-control-sm line-total bg-light" readonly></td>
                                            <td><button type="button" class="btn btn-sm btn-outline-danger line-del"><i class="bi bi-x-lg"></i></button></td>
                                        </tr>
                                    <?php endif; ?>
                    ...
                    <div class="col-md-7">
                        ...
                        <textarea name="note" class="form-control rounded-3" rows="3" maxlength="1000"><?= htmlspecialchars($currentNote, ENT_QUOTES, 'UTF-8') ?></textarea>
                    </div>
                    <div class="col-md-5">
                    ...
                    <div class="col-12 d-flex gap-2 flex-wrap">
                        ...
                        <?php if ($isEditing): ?>
                            <a href="<?= htmlspecialchars(app_path('pages/purchase-bill.php') . '?month=' . rawurlencode($month), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-secondary rounded-pill">ยกเลิกแก้ไข</a>
                        <?php else: ?>
    ...
    <div class="card po-card">
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-end flex-wrap gap-2 mb-3">
                <h5 class="fw-bold mb-0">รายการบิลซื้อ เดือน <?= htmlspecialchars($month, ENT_QUOTES, 'UTF-8') ?></h5>
            </div>
            ...
            <form method="get" class="row g-2 align-items-center mb-3">
                ...
                <div class="col-12 col-md-3 d-flex gap-2">
                    ...
                    <a href="<?= htmlspecialchars(app_path('pages/purchase-bill.php') . '?month=' . rawurlencode($month), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-outline-secondary" title="ล้างตัวกรอง" aria-label="ล้างตัวกรอง">
                        ...
                        <?php else: ?>
                            <?php foreach ($pagedRows as $row): ?>
                                ...
                                <tr>
                                    ...
                                    <td class="small"><?= htmlspecialchars((string) ($row['vendor_name'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td>
                                    <td class="small"><?= htmlspecialchars((string) ($row['site_label'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td>
                                    <td class="text-end fw-bold">฿<?= number_format((float) ($row['total_amount'] ?? 0), 2) ?></td>
                                    <td class="text-center">
                                        <div class="d-inline-flex gap-1">
                                            <?php if ($isAdmin || (int) ($row['created_by'] ?? 0) === $me): ?>
                                                <a
                                                    href="<?= htmlspecialchars(app_path('pages/purchase-bill.php') . '?' . http_build_query(array_merge($qsBase, ['edit' => (int) ($row['id'] ?? 0)])), ENT_QUOTES, 'UTF-8') ?>"
                                            ...
                                            <?php endif; ?>
                                            <?php if ($canDelete): ?>
                                                <a href="<?= htmlspecialchars($handler . '?' . http_build_query(array_merge($qsBase, ['action' => 'delete', 'id' => (int) ($row['id'] ?? 0)])) . $csrfQ, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('ยืนยันลบบิลนี้ ?');" title="ลบบิล">
            ...
            <?php if ($billCount > 0): ?>
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mt-3">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <a href="<?= htmlspecialchars(app_path('pages/purchase-bill.php') . '?' . http_build_query($prevQuery), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-outline-secondary<?= $page <= 1 ? ' disabled' : '' ?>"<?= $page <= 1 ? ' tabindex="-1" aria-disabled="true"' : '' ?>>ดูก่อนหน้า</a>
                        <span class="small text-muted">หน้า <?= number_format($page) ?>/<?= number_format($totalPages) ?></span>
                        <a href="<?= htmlspecialchars(app_path('pages/purchase-bill.php') . '?' . http_build_query($nextQuery), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-outline-secondary<?= $page >= $totalPages ? ' disabled' : '' ?>"<?= $page >= $totalPages ? ' tabindex="-1" aria-disabled="true"' : '' ?>>ดูถัดไป</a>
                    </div>
                </div>
            <?php endif; ?>
        </div>
...
<template id="lineTpl">
    <tr class="line-row">
        ...
        <td><input type="text" class="form-control form-control-sm line-name" name="item_name[]" maxlength="500" required></td>
        <td><input type="number" class="form-control form-control-sm line-qty" name="item_qty[]" step="0.001" min="0" value="1" required></td>
        <td><input type="text" class="form-control form-control-sm line-unit" name="item_unit[]" maxlength="40" placeholder="หน่วย"></td>
        <td><input type="number" class="form-control form-control-sm line-price" name="item_price[]" step="0.001" min="0" required></td>
        <td><input type="text" class="form-control form-control-sm line-discount" name="item_discount[]" maxlength="20"></td>
        <td><input type="text" class="form-control form-control-sm line-total bg-light" readonly></td>
        <td><button type="button" class="btn btn-sm btn-outline-danger line-del"><i class="bi bi-x-lg"></i></button></td>
...
(function() {
    ...
    if (q.get('err')) {
        ...
        Swal.fire({ icon: 'error', title: 'ไม่สามารถดำเนินการได้', text: map[q.get('err')] || q.get('err'), confirmButtonColor: '#0d6efd' });
    }
})();

...
<div class="container mt-4 mb-5">
    ...
    <?php if (!empty($_GET['line_error']) && $_GET['line_error'] === '1'): ?>
        <div class="alert alert-warning alert-dismissible fade show" role="alert">
            บันทึกคำขอแล้ว แต่ส่ง LINE ไม่สำเร็จ กรุณาลองส่งใหม่อีกครั้ง
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    <?php endif; ?>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex align-items-center gap-2">
            <?php if ($isAdmin): ?>
                <?php if ($showAll): ?>
                    <a href="<?= htmlspecialchars(app_path('pages/leave-request-list.php'), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-dark rounded-pill px-3">
                        <i class="bi bi-person me-1"></i> เฉพาะของฉัน
                    </a>
                <?php else: ?>
                    <a href="<?= htmlspecialchars(app_path('pages/leave-request-list.php') . '?scope=all', ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-dark rounded-pill px-3">
                        <i class="bi bi-collection me-1"></i> โชว์ใบลาทั้งหมด
                    </a>
                <?php endif; ?>
            <?php endif; ?>
            <a href="<?= htmlspecialchars(app_path('pages/leave-request-create.php'), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-orange rounded-pill px-4 shadow-sm">
                    ...
                    <?php foreach ($allRows as $row): ?>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="<?= htmlspecialchars(app_path('pages/leave-request-view.php'), ENT_QUOTES, 'UTF-8') ?>?id=<?= (int) ($row['id'] ?? 0) ?>" class="btn btn-sm btn-outline-primary" title="ดูรายละเอียด">
                                    <i class="bi bi-eye-fill"></i>
                                </a>
                                <?php if ($status === 'draft' && (int) ($row['requested_by'] ?? 0) === $me): ?>
                                    <a href="<?= htmlspecialchars(app_path('actions/action-handler.php'), ENT_QUOTES, 'UTF-8') ?>?action=send_leave_request&id=<?= (int) ($row['id'] ?? 0) ?><?= htmlspecialchars($csrfQ, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-outline-success" title="ส่งใบลาไปอนุมัติ" onclick="return confirm('ยืนยันการส่งใบลานี้ไป LINE ?')">

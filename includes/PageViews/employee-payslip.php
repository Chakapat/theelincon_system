...
$employee = $uid > 0 ? Db::row('users', (string) $uid) : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_payslip_request') {
        ...
        if ($employeeId > 0 && preg_match('/^\d{4}-\d{2}$/', $periodPost) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $payDatePost)) {
            $emp = Db::row('users', (string) $employeeId);
            if ($emp !== null) {
                ...
                header('Location: ' . app_path('pages/employee-payslip-request-list.php') . '?created=1');
                exit();
            }
        }
        header('Location: ' . app_path('pages/employee-payslip.php') . '?err=1');
        exit();
    }
}
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-4">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
                ...
                <a href="<?= htmlspecialchars(app_path('pages/employee-payslip-request-list.php'), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-dark">รายการคำขอสลิป</a>
            </div>

...
function nextTaxInvoiceNumber(string $seedDate): string
{
    ...
    foreach (Db::tableRows('tax_invoices') as $row) {
        $num = strtoupper((string) ($row['tax_invoice_number'] ?? ''));
...
<?php if ($taxHasAlerts): ?>
<div class="doc-view-alerts no-print">
...
document.addEventListener('DOMContentLoaded', function () {
    Swal.fire({
        ...
        window.location.href = <?= json_encode(app_path('pages/invoices/tax-invoice-list.php'), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?>;
    });
});
</script>
<?php endif; ?>

...
<div class="container mt-4 mb-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="<?= htmlspecialchars(app_path('pages/advance-cash/advance-cash-list.php'), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-secondary rounded-pill px-4">กลับรายการ</a>
    </div>
    ...
    <div class="card detail-card p-4">
        ...
        <?php if ($isFinanceRole): ?>
            ...
            <?php if ($status === 'approved'): ?>
                <a href="<?= htmlspecialchars(app_path('pages/advance-cash/advance-cash-receipt.php') . '?id=' . (int) ($row['id'] ?? 0), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-primary px-4">
                    <i class="bi bi-receipt me-1"></i> ออกใบเสร็จรับเงิน
                </a>
            <?php endif; ?>
            <?php if ((string) ($row['receipt_status'] ?? 'none') === 'issued'): ?>
                <a href="<?= htmlspecialchars(app_path('pages/advance-cash/advance-cash-receipt-print.php') . '?id=' . (int) ($row['id'] ?? 0), ENT_QUOTES, 'UTF-8') ?>" class="btn btn-outline-dark px-4" target="_blank" rel="noopener">

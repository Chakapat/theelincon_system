...
if ($isAdmin && $editId > 0) {
    $er = Db::row('internal_announcements', (string) $editId);
    if ($er !== null) {
        $editRow = [
            'id' => $er['id'] ?? $editId,
            'title' => $er['title'] ?? '',
        ...
        <div class="card-body p-4">
            ...
            <form method="post" action="<?= htmlspecialchars($handler) ?>?action=save">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    ...
                    <?php if ($editRow): ?>
                        <a href="<?= htmlspecialchars(app_path('pages/announcements.php')) ?>" class="btn btn-outline-secondary rounded-pill">ยกเลิกการแก้ไข</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_name'])) {
    ...
    if ($n !== '' && strlen($n) <= 200) {
        $nid = Db::nextNumericId('cash_ledger_stores', 'id');
        Db::setRow('cash_ledger_stores', (string) $nid, [
            'id' => $nid,
            'name' => $n,
            'sort_order' => 0,
            'is_active' => 1,
    ...
    header('Location: ' . app_path('pages/cash-ledger-master-stores.php') . '?ok=1');
    exit;
}
...
if (isset($_GET['toggle'])) {
    $id = (int) $_GET['toggle'];
    if ($id > 0) {
        $cur = Db::row('cash_ledger_stores', (string) $id);
        if ($cur !== null) {
            $active = !empty($cur['is_active']) ? 1 : 0;
            Db::mergeRow('cash_ledger_stores', (string) $id, ['is_active' => $active ? 0 : 1]);
        }
    }
    header('Location: ' . app_path('pages/cash-ledger-master-stores.php'));
    exit;
}

$list = Db::tableRows('cash_ledger_stores');
...
<head>
    ...
    <title>ร้านค้า (รายรับรายจ่าย) | THEELIN CON</title>
...
<div class="container pb-5">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
        <h4 class="fw-bold mb-0"><i class="bi bi-shop me-2 text-warning"></i>ร้านค้า / แหล่งซื้อ (ไม่เกี่ยว PR/PO)</h4>
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-4">
            <h6 class="fw-bold mb-3">เพิ่มร้านใหม่</h6>
            <form method="post" class="row g-2 align-items-end">
                <div class="col-md-8">
                    <label class="form-label small">ชื่อร้าน / แหล่งซื้อ</label>
                    <input type="text" name="add_name" class="form-control rounded-3" maxlength="200" required placeholder="เช่น โฮมโปร บางนา">
                </div>
                <div class="col-md-4">

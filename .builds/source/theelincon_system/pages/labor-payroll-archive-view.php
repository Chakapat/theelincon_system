...
<div class="container pb-5">
    <div class="mb-3 mt-2 no-print d-flex flex-wrap align-items-center gap-2">
        ...
        <?php if ($head): ?>
        <a href="<?= htmlspecialchars(app_path('pages/labor-payroll-archive-edit.php') . '?id=' . (int) $id, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-sm btn-outline-warning rounded-pill"><i class="bi bi-pencil me-1"></i>แก้ไข</a>
            ...
            <div class="card card-v mb-4 archive-summary-block">
                <div class="card-body">
                    ...
                    <div class="row g-2 g-md-3 small">
                        ...
                        <div class="col-md-3"><span class="text-muted">วันที่</span><br><strong><?= htmlspecialchars(date('d/m/Y H:i', strtotime((string) $head['closed_at'])), ENT_QUOTES, 'UTF-8') ?></strong></div>
                        ...
                        <div class="col-md-4">
                            ...
                            <strong>฿<?= number_format((float) $head['total_gross'], 2) ?></strong>
                        </div>
                        <div class="col-md-4">
                            ...
                            <strong class="text-success fs-5">฿<?= number_format((float) $head['total_net'], 2) ?></strong>
                        </div>
                    </div>
                </div>
            </div>

            <div class="table-responsive card card-v">
                <table class="table table-sm table-bordered mb-0 align-middle table-print">
                    <thead class="table-light">
                        <tr>
                            <th style="width:28%">ชื่อคนงาน</th>
                            <th class="text-center">วันมา</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?= htmlspecialchars((string) $ln['worker_name'], ENT_QUOTES, 'UTF-8') ?></td>
                            <td class="text-center"><?= (int) $ln['days_present'] ?></td>
                            <td class="text-center"><?= number_format((float) $ln['ot_hours'], 2) ?></td>
                            <td class="text-end"><?= number_format((float) $ln['daily_wage'], 2) ?></td>
                            <td class="text-end"><?= number_format((float) $ln['advance_draw'], 2) ?></td>
                            <td class="text-end"><?= number_format((float) $ln['gross_amount'], 2) ?></td>
                            <td class="text-end fw-semibold"><?= number_format((float) $ln['net_amount'], 2) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>

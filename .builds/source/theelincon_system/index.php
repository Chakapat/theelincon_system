...
<div class="container pb-5">
    <section class="home-menu-hub mb-4" aria-label="เมนูระบบ">
            ...
            <div class="col">
                <div class="card home-hub-card border-0 shadow-sm h-100 rounded-4 overflow-hidden">
                    ...
                    <div class="card-body p-3 d-grid gap-2">
                        ...
                        <a class="home-hub-link d-flex align-items-center" href="<?= htmlspecialchars(app_path('pages/cash-ledger-master-stores.php'), ENT_QUOTES, 'UTF-8') ?>"><i class="bi bi-shop me-2 text-secondary"></i>+ ร้านค้า</a>
                        <a class="home-hub-link d-flex align-items-center" href="<?= htmlspecialchars(app_path('pages/cash-ledger-master-sites.php'), ENT_QUOTES, 'UTF-8') ?>"><i class="bi bi-geo-alt me-2 text-secondary"></i>+ ไซต์งาน</a>
                    </div>
                </div>
            </div>
            <div class="col">
        ...
        <div class="col-md-8">
            <div class="card card-stats border-0 shadow-sm p-3 rounded-4" style="border-left-color: #198754;">
                <div class="d-flex align-items-center">
                    ...
                    <div class="ms-3">
                        <h6 class="text-muted mb-0">ประมาณการยอดเงิน</h6>
                ...
                <div class="col-md-7 text-end">
                    <div class="d-flex gap-2 justify-content-md-end">
                        <div class="position-relative flex-grow-1" style="max-width: 300px;">
                            ...
                            <input type="text" id="search_invoice" class="form-control ps-5 rounded-pill border-light-subtle bg-light" placeholder="ค้นหาเลขที่บิล">
                        </div>
                        <a href="<?= htmlspecialchars(app_path('pages/invoice-create.php')) ?>" class="btn btn-orange rounded-pill px-4 shadow-sm">
...
function loadTable(query = '') {
    ...
    fetch(`${indexUrl}?ajax_search=1&search=${encodeURIComponent(query)}`)
        .then(res => res.text())
        .then(data => {
...
let searchTimeout;
document.getElementById('search_invoice').addEventListener('input', function() {
...
function deleteItem(id, type) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = `${actionHandlerUrl}?action=delete&type=${type}&id=${id}`;
        }
    });
}
